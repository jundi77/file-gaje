...
class base {
	...
	curl_setopt_array($curl, array(
	  CURLOPT_URL => "http://my.its.ac.id/userinfo",
	...
	//error_log($response.PHP_EOL, 3, "/var/lmsdata/login.log");
	...
	$response = json_decode($response);
        ...
        $userinfo = [
            ...
            'idnumber' => $response->reg_id,
	    'alternate_name' => $response->nickname,
...
//        $email = $idtoken->claim('email');
	...
	$alt_email = $response->alternate_email;
        if (!empty($email)) {
            $userinfo['email'] = $email;
        }else{
	    $userinfo['email'] = $alt_email;
	}
